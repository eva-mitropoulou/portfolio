...
Built a retrospective public-data HTE reaction-yield workflow with reaction cleaning, component featurization, random and out-of-component validation, uncertainty calibration, active-learning simulation, and existing-record ranking.
...
- Categorical component one-hot featurization.
...
- Ensemble-variance and conformal-style uncertainty diagnostics.
- Budgeted active-learning simulation over existing records.
- Existing-record ranking with confidence and domain warnings.

## 5. Validation Strategy

Primary model selection used the grouped high-cardinality component split. Random split performance is reported but not treated as sole evidence.
...
Supported metrics from `reaction-yield-ml/reports/metrics/summary.json`:
