...
from reaction_yield_ml.reporting.run_state import update_run_state
from reaction_yield_ml.reporting.io import read_json, write_json, write_markdown


def parse_args() -> argparse.Namespace:
    return argparse.ArgumentParser(description="Run final public-portfolio quality gate.").parse_args()
...
def run_quality_gate() -> dict[str, Any]:
    ...
    checks = {
        ...
        "portfolio_page_updated_only_if_project_credible": _file_exists("../projects/reaction-yield-ml.md") or _file_exists("projects/reaction-yield-ml.md"),
        "cv_bullets_evidence_backed": _file_exists("../portfolio_assets/cv_bullets_reaction_yield_ml.md") or _file_exists("portfolio_assets/cv_bullets_reaction_yield_ml.md"),
    ...
    payload = {
        ...
        "remaining_limitations": [
            ...
            "Portfolio/CV wording should remain metric-free unless supported by final_summary.json.",
        ],
    }
    write_json(METRICS_DIR / "final_quality_gate_report.json", payload)
    report = "# Final Quality Gate Report\n\n" + "\n".join(
