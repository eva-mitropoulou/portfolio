...
|---|---|---|---|
| Built a retrospective EGFR CADD and QSAR decision workflow from ChEMBL, curating 26,600 IC50 records into 10,593 model-ready molecules; benchmarked RDKit descriptor, Morgan fingerprint, and exploratory custom PyTorch GCN baselines under random and scaffold splits, with Morgan RF achieving scaffold-split RMSE 0.871 and R2 0.550; quantified applicability-domain degradation from high-similarity MAE 0.513 to low-similarity MAE 0.957; added assay-aware validation, conformal-style uncertainty checks, ADMET-style and model-risk triage, SAR and error analysis, active-learning simulation, CLI prediction, ligand-contact analysis across four EGFR PDB structures, and 5UG9 redocking pose-recovery audit at 0.968 A RMSD. | `egfr-cadd-qsar-admet/reports/final_egfr_cadd_qsar_report.md` | `egfr-cadd-qsar-admet/reports/metrics/egfr_final_hardening_status.json` | safe |
...
| Framed antibody outputs as retrospective benchmarking and existing-record prioritization, explicitly separating public-label model triage from sequence design, optimization, or prospective validation. | `projects/antibody-sequence-ml.md` | `portfolio_audit/evidence_map.json` | safe |
